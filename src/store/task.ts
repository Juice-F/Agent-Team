 ...
 *
 * 这东西不属于哪个角色——它是在流程里流动的那张单子，所以放在 store 而不是
 * 某个 agent 底下。各角色自己的中间产物（产品的 tasks.md、研发的 diff）形状
 * 差得远，各存各的，别往这里塞。
 */
...
export const TaskSchema = z.object({
  ...
  /** 流程走到哪了。图在 src/index.ts，谁认领这个阶段引擎那儿查。 */
...
export async function listTasks(): Promise<Task[]> {
  ...
  for (const file of files) {
    ...
    const task = await loadTask(file.slice(0, -".json".length));
    if (task) tasks.push(task);
  }
  return tasks;
}

/** 引擎要的那三样，凑一起传进去，省得 workflow 反过来 import store */
